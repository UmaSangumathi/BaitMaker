...
Simple bash script that calls bam2cons-stringent.pyrepeatedly until the maximum
...
# 
which bwa_unique-stringent.sh >/dev/null || exit 1
which samtools >/dev/null || exit 1
which bam2cons-stringent.py>/dev/null || exit 1
bam2cons-stringent.py >/dev/null
if [ $? -ne 0 ]; then
    echo "FATAL: dry run of bam2cons-stringent.pyfailed. Is pysam installed?" 1>&2
    exit 1
fi
...
while [ $iter -le $MAX_ITER ]; do
    ...
    echo "INFO: calling bwa_unique-stringent.sh with the following args: $bwa_unique_args"
    #test $DEBUG -eq 1 && echo "DEBUG iter $iter: bwa_unique_args=$bwa_unique_args" 1>&2
    bwa_unique-stringent.sh $bwa_unique_args || exit 1
    ...
    test $DEBUG -eq 1 && echo "DEBUG iter $iter: calling bam2cons-stringent.py-b $bam -r $seq -o $new_ref --no-cov-char N" 1>&2
    bam2cons-stringent.py-b $bam -r $seq -o $new_ref --no-cov-char N -v || exit 1
