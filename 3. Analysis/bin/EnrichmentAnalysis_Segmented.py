#!/usr/bin/python
##############################################################
#  Script     : EnrichmentAnalysis_Segmented.py 
...
# Read the summary file:
  ...
  if f1 in info_dict: 
    ...
    print "Information file error: The fastq file not present in the file" 
  all_segs_ref = ref_dir + "/" + ref_id 
  threads = str(3)
 ...
 # Log file
  ...
  for x in SeqIO.parse(open(all_segs_ref, "r"), "fasta"):
    ref = res_dir + "/" + x.id.replace("_", "").replace("|","").replace("/","") + ".ref"
    with open(ref, "w") as ff:
      ff.write(">" + x.id + "\r\n" + str(x.seq) + "\r\n")
 
    #1- Consensus: Map reads to a reference genome...
    cons_ref = res_dir + "/" + x.id + "-1.fa"  
   ...
   # Blast the consensus formed.... 
    if os.stat(cons_ref).st_size == 0: 
      print "NO COVERAGE ", x.id
    elif [len(str(x.seq)) for x in SeqIO.parse(cons_ref, "fasta")][0] < 50:
      print "Less than 50bp sequence" 
    else:
   ...
   # 2-iteration Map reads to nearest reference...
      cons_ref2 = res_dir + "/" + x.id  +  "-cons-2.fa" 
...
# get Hybrid reference genome....
      ...
      hybrid_reffile = res_dir + "/" + f1.replace(".fq", "-hybrid.fa")
      ...
      with open(hybrid_reffile.replace(".fa", ".annot"), "a") as f:
        for h in from_reference_gp:
          f.write(x.id + "\t" + "From reference used: " + "\t" + str(h[0]) + "-" + str(h[1]) + "\r\n")
        for h in from_consenses_gp:
          f.write(x.id + "\t" + "From consensus generated: " + "\t" + str(h[0]) + "-" + str(h[1]) + "\r\n")
      new_ref = "".join(hybrid_ref)  
      with open( hybrid_reffile , "a") as f: 
        f.write(">Hybrid_consensus|" + x.id + "\r\n" + new_ref + "\r\n")  
  
# Put all segments ina single file...
