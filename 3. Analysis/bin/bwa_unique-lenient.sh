...
bwa_aln_extra_args="-t $threads -n 20 -q 3 "
# -n: percentage of mismatches... default 0.04...  uma edited
# -t: number of threads
...
bwa $args > test.sam
bwa $args |  grep '\(^@\|XT:A:U\)' |  samtools view -b -S -F 12 - | java -jar $picard_readgroup_jar INPUT=/dev/stdin OUTPUT=/dev/stdout  VALIDATION_STRINGENCY=LENIENT  RGCN="GIS" RGPL="illumina" RGLB="library-placeholder"  RGPU="platform-unit-placeholder" RGSM="sample-name-placeholder"  |  samtools sort - ${outbam%.bam}  || exit 1
#EOF
